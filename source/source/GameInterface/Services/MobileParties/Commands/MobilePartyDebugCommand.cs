...
internal static class MobilePartyDebugCommand
{
    ...
    // coop.debug.mobileparty.set_field <PartyId> <FieldName> <Value>
    [CommandLineArgumentFunction("set_field", "coop.debug.mobileparty")]
    public static string SetField(List<string> args)
    {
        if (args.Count != 3)
        {
            return "Usage: coop.debug.mobileparty.set_field <PartyId> <FieldName> <Value>";
        }
        ...
        FieldInfo field = AccessTools.Field(typeof(MobileParty), fieldName);
        if (field == null)
        {
            return $"Field '{fieldName}' not found in MobileParty.";
        }
        ...
        try
        {
            ...
            return $"Set MobileParty {partyId} field {fieldName} to {value}.";
        }
        catch (Exception ex)
        {
            return $"Error setting field: {ex.Message}";
        }
    }
}
